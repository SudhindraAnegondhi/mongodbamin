...
class ModelGenerator {
  ...
  ModelGenerator(this._rootClassName, privateFields, List<String> dateFields,
      primaryKey, foreignKeys, noUpdate, index)
      : _privateFields = privateFields ?? false {
    if (dateFields != null) {
      hints = [];
      dateFields.forEach((path) {
        ...
        hints.add(Hint(path, 'DateTime'));
      });
    } else {
      this.hints = new List<Hint>();
    }
    customization.primaryKey = primaryKey ?? '';
    customization.foreignKeys =Map<String, dynamic>.from( foreignKeys ?? {});
    customization.noUpdate = noUpdate ?? [];
    customization.index = List<Map<String, dynamic>>.from(index ?? []);
  }

  Hint _hintForPath(String path) {
    final hint = this.hints.firstWhere(
        (h) => h.path.toLowerCase() == path.toLowerCase(),
        orElse: () => null);
    return hint;
  }
  ...
  List<Warning> _generateClassDefinition(
    ...
    if (jsonRawDynamicData is List) {
      ...
      ClassDefinition classDefinition =
          new ClassDefinition(className, _privateFields, customization);
      keys.forEach((key) {
      ...
      dependencies.removeWhere((element) => element.typeDef.name == 'DateTime');
